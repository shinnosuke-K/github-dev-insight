...
func (rq *RepositoryQuery) QueryPullRequests() *PullRequestQuery {
	query := &PullRequestQuery{config: rq.config}
...
func (rq *RepositoryQuery) QueryIssues() *IssueQuery {
	query := &IssueQuery{config: rq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := rq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (rq *RepositoryQuery) sqlAll(ctx context.Context) ([]*Repository, error) {
	...
	if query := rq.withPullRequests; query != nil {
		...
		for i := range nodes {
			...
			nodes[i].Edges.PullRequests = []*PullRequest{}
		}
		query.withFKs = true
		query.Where(predicate.PullRequest(func(s *sql.Selector) {
			s.Where(sql.InValues(repository.PullRequestsColumn, fks...))
		}))
	...
	if query := rq.withIssues; query != nil {
		...
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Issues = []*Issue{}
		}
		query.withFKs = true
		query.Where(predicate.Issue(func(s *sql.Selector) {
			s.Where(sql.InValues(repository.IssuesColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err

...
func (prc *PullRequestCreate) Save(ctx context.Context) (*PullRequest, error) {
	prc.defaults()
	return withHooks[*PullRequest, PullRequestMutation](ctx, prc.sqlSave, prc.mutation, prc.hooks)
}
...
func (prc *PullRequestCreate) check() error {
	if _, ok := prc.mutation.GithubID(); !ok {
		return &ValidationError{Name: "github_id", err: errors.New(`ent: missing required field "PullRequest.github_id"`)}
	}
	...
	if _, ok := prc.mutation.TotalCommits(); !ok {
		return &ValidationError{Name: "total_commits", err: errors.New(`ent: missing required field "PullRequest.total_commits"`)}
	}
	...
	if _, ok := prc.mutation.GetCommit(); !ok {
		return &ValidationError{Name: "get_commit", err: errors.New(`ent: missing required field "PullRequest.get_commit"`)}
	}
	if _, ok := prc.mutation.CreatedAt(); !ok {

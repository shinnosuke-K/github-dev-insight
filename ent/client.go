...
func (c *Client) Intercept(interceptors ...Interceptor) {
	c.Commits.Intercept(interceptors...)
...
func (c *Client) Mutate(ctx context.Context, m Mutation) (Value, error) {
	switch m := m.(type) {
	case *CommitsMutation:
		return c.Commits.mutate(ctx, m)
	case *IssueMutation:
...
// A call to `Intercept(f, g, h)` equals to `commits.Intercept(f(g(h())))`.
func (c *CommitsClient) Intercept(interceptors ...Interceptor) {
	c.inters.Commits = append(c.inters.Commits, interceptors...)
}
...
func (c *CommitsClient) Query() *CommitsQuery {
	return &CommitsQuery{
		config: c.config,
		ctx:    &QueryContext{Type: TypeCommits},
...
func (c *CommitsClient) Interceptors() []Interceptor {
	return c.inters.Commits
}

func (c *CommitsClient) mutate(ctx context.Context, m *CommitsMutation) (Value, error) {
	switch m.Op() {
	case OpCreate:
		return (&CommitsCreate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdate:
		return (&CommitsUpdate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdateOne:
		return (&CommitsUpdateOne{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpDelete, OpDeleteOne:
		return (&CommitsDelete{config: c.config, hooks: c.Hooks(), mutation: m}).Exec(ctx)
	default:
		return nil, fmt.Errorf("ent: unknown Commits mutation op: %q", m.Op())
	}
}
...
func (c *PullRequestClient) QueryCommits(pr *PullRequest) *CommitsQuery {
	query := (&CommitsClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := pr.ID
		step := sqlgraph.NewStep(
...
type (
	hooks struct {
		Commits, Issue, PullRequest, Repository []ent.Hook
	}
	inters struct {
		Commits, Issue, PullRequest, Repository []ent.Interceptor
	}
)
